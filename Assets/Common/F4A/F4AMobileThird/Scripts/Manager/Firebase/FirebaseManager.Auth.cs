namespace com.F4A.MobileThird
{
    ...
    {
        public static event System.Action<bool> OnLoginFacebookCompleted = delegate { };
        ...
        protected void InitializeAuth()
        {
            ...
            _firebaseApp.SetEditorDatabaseUrl(_firebaseInfo.Urldatabase);
        ...
        {
#if DEFINE_FIREBASE_AUTH
            return _user != null && !string.IsNullOrEmpty(_user.UserId);
#endif
            return false;
        }
            ...
            try
            {
                if (!string.IsNullOrEmpty(accessToken))
                {
                    ...
                    _auth.SignInWithCredentialAsync(credential).ContinueWithOnMainThread(task =>
                    {
                        if (task.IsCanceled)
                        {
                            Debug.LogError("FirebaseManager.SignInWithFacebook/SignInWithCredentialAsync was canceled.");
                            if (callDelegate) OnLoginFacebookCompleted?.Invoke(false);
                            return;
                        }
                        if (task.IsFaulted)
                        {
                            Debug.LogError("FirebaseManager.SignInWithFacebook/SignInWithCredentialAsync encountered an error: " + task.Exception);
                            if (callDelegate) OnLoginFacebookCompleted?.Invoke(false);
                            return;
                        }

                        _user = task.Result;
                        Debug.LogFormat("FirebaseManager.SignInWithFacebook User signed in successfully: {0} ({1})", _user.DisplayName, _user.UserId);
                        if (callDelegate) OnLoginFacebookCompleted?.Invoke(true);
                    });
#endif
                }
            }
        ...
        public void SaveDatabase(string databaseName, string json)
        {
            ...
            if (IsSignIn())
            {
                ...
                _referenceDatabase.UpdateChildrenAsync(childUpdates);
            }
#endif
        }
        ...
        public void SaveUserData(string databaseName, string json, System.Action callBack = null)
        {
#if DEFINE_FIREBASE_AUTH
            if (IsSignIn())
            {

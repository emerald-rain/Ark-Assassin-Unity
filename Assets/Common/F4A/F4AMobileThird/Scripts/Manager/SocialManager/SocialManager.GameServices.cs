    ...
    public partial class SocialManager
	{
        ...
        private void ProcessAuthentication(bool success)
        {
            ...
            try
            {
                ...
                if (OnAuthenticateGameServices != null)
                {
                    OnAuthenticateGameServices(success);
                }
#if UNITY_ANDROID
                if (!success)
                {
...
#elif UNITY_IOS
	        id = id.Replace('-', '_');
	        if(Social.Active != null && Social.localUser.authenticated && IsLoginGameServiceSuccess)
	        {
        ...
        /// <param name="levelCurrent">Level current.</param>
        /// <param name="levelUnlock">Level unlock.</param>
        public void CheckUnlockAchievement(string id, int levelCurrent, int levelUnlock)
        {
            if(!CPlayerPrefs.GetBool(id, false) && levelCurrent >= levelUnlock)
            {

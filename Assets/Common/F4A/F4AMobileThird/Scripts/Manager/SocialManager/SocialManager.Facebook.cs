...
namespace com.F4A.MobileThird
{
    public partial class SocialManager
    {
        ...
        public static event Action<IGraphResult> OnGetPlayerPicture = delegate { };
...
#pragma warning disable 219
        ...
        private void InitFacebook()
        {
            ...
            {
                strQueryFields = "id,email, name,first_name,last_name";
            }
        ...
        private void GetInfoFacebookUser_CallBack(IGraphResult result)
        {
            ...
            OnGetInfoFacebookUserCompleted();
        }
#endif

        public void GetInfoFacebookUser()
        {
            if (socialSettingInfo.isGetUserInfoFacebook)
            {
#if DEFINE_FACEBOOK_SDK
...
#region PlayerInfo
        ...
        {
#if DEFINE_FACEBOOK_SDK
            FB.API(GraphUtil.GetPictureQuery("me", 128, 128), HttpMethod.GET, HandleGetPlayerPicture);
#endif
        }
        ...
        {
            if(OnGetPlayerPicture != null)
            {
                OnGetPlayerPicture(result);
            }
...
#region Load Image
        ...
        public string GetFieldFacebook(EFieldQueryType type)
        {
            switch (type)
            {
...
#region Take screenshot
        ...
        public void ShareNative(string filePath, string msg, string title)
        {
            if (!_isProcessingShareNative)
            {
                StartCoroutine(IEShareScreenshot(filePath, msg, title));
            }
        }
        ...
        private IEnumerator IEShareScreenshot(string filePath = "", string msg = "", string title = "")
        {
            ...
            string linkStore = socialSettingInfo.GetUrlDeveloperOnStore();
            string nameGame = BuildManager.Instance.GetGameName();
            ...
            if(!string.IsNullOrEmpty(filePath)) new NativeShare().AddFile(filePath).SetSubject(title).SetText(msg).Share();
